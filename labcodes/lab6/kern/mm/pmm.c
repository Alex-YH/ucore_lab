...
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end,
	...
	do {
		...
		if (*ptep & PTE_P) {
			...
			void * src_kv_addr = page2kva(page);
			void * dst_kv_addr = page2kva(npage);

			memcpy(dst_kv_addr, src_kv_addr, PGSIZE);
...
void print_pgdir(void) {
	...
	while ((perm = get_pgtable_items(0, NPDEENTRY, right, vpd, &left, &right))
		...
		while ((perm = get_pgtable_items(left * NPTEENTRY, right * NPTEENTRY,
				r, vpt, &l, &r)) != 0) {
